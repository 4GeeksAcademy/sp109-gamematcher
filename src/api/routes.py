...
def create_game():
    ...
    # 2) Duplicados (prioriza rawg_id; si no hay, por nombre case-insensitive)
    ...
    db.session.flush()  # obtener new_game.id antes de relacionar

    # 4) Relación con plataformas (por IDs o por nombres)
    ...
    if platform_ids:
        for pid in platform_ids:
            ...
            if platform:
                ...
                print(f"⚠️ Plataforma con id {pid} no encontrada")
    elif platforms_names:
        for pname in platforms_names:
            platform = Platform.query.filter(
                func.lower(Platform.name) == pname.lower()
            ).first()
            if platform:
                db.session.add(GamePlatform(game_id=new_game.id, platform_id=platform.id))
            else:
                print(f"⚠️ Plataforma con nombre '{pname}' no encontrada")

    # 5) Relación con géneros (por IDs o por nombres)
    ...
    if genre_ids:
        for gid in genre_ids:
            ...
            if genre:
                ...
                print(f"⚠️ Género con id {gid} no encontrado")
    elif genres_names:
        for gname in genres_names:
            genre = Genre.query.filter(
                func.lower(Genre.name) == gname.lower()
            ).first()
            if genre:
                db.session.add(GameGenre(game_id=new_game.id, genre_id=genre.id))
            else:
                print(f"⚠️ Género con nombre '{gname}' no encontrado")
...
def create_platform():
    ...
    existing = Platform.query.filter(func.lower(Platform.name) == name.lower()).first()
    if existing:
        return jsonify(existing.serialize()), 200
    ...
    new_platform = Platform(
        ...
        image=data.get("image")  # <<--- NUEVO: soporta imagen
    )
...
def update_platform(platform_id):
    ...
    platform.image = data.get("image", platform.image)  # <<--- NUEVO: soporta imagen
    db.session.commit()
    return jsonify(platform.serialize()), 200
...
def create_game_genre():
    ...
    existing = GameGenre.query.filter_by(game_id=game_id, genre_id=genre_id).first()
    if existing:
        return jsonify(existing.serialize()), 200
...
def create_user_platform_preference():
    ...
    new_preference = UserPlatformPreference(user_id=user_id, platform_id=platform_id)
...
def verify_token():
    ...
    admin = db.session.execute(
        select(AdminUser).where(AdminUser.name == current_identity)
    ).scalar_one_or_none()
    if admin:
        return jsonify({
            "id": admin.id,
    ...
    user = db.session.execute(
        select(User).where(User.nickname == current_identity)
    ).scalar_one_or_none()
    if user:
        return jsonify({
            "id": user.id,
...
def get_user_excluded_rawg_games(user_id):
    ...
    try:
        ...
        excluded_rawg_ids.extend([non_fav for non_fav in non_favorites if non_fav])

        excluded_rawg_ids = list(set(excluded_rawg_ids))

        print(f"🔍 Usuario {user_id}: {len(excluded_rawg_ids)} juegos excluidos por RAWG ID")
...
def get_game_recommendations_context():
    ...
    try:
        ...
        user = db.session.execute(
            select(User).where(User.nickname == current_user_identity)
        ).scalar_one_or_none()
